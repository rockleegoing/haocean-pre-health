...
// 批量添加任务
func (task Task) BatchAdd(tasks []monitor.SysJob) {
...
// 删除任务后添加
func (task Task) RemoveAndAdd(taskModel monitor.SysJob) {
	task.Remove(taskModel.JobId)
	task.Add(taskModel)
}

// 添加任务
func (task Task) Add(taskModel monitor.SysJob) {
...
func (task Task) NextRunTime(taskModel monitor.SysJob) time.Time {
...
// 直接运行任务
func (task Task) Run(taskModel monitor.SysJob) {
	go createJob(taskModel)()
}

type Handler interface {
	Run(taskModel monitor.SysJob, taskUniqueId int64) (string, error)
}
...
func (h *HTTPHandler) Run(taskModel monitor.SysJob, taskUniqueId int64) (result string, err error) {
...
func (h *RPCHandler) Run(taskModel monitor.SysJob, taskUniqueId int64) (result string, err error) {
...
// 创建任务日志
func createTaskLog(taskModel monitor.SysJob, status int) (int64, error) {
...
func createJob(taskModel monitor.SysJob) cron.FuncJob {
...
func createHandler(taskModel monitor.SysJob) Handler {
...
// 任务前置操作
func beforeExecJob(taskModel monitor.SysJob) (taskLogId int64) {
...
// 任务执行后置操作
func afterExecJob(taskModel monitor.SysJob, taskResult TaskResult, taskLogId int64) {
...
// 执行依赖任务, 多个任务并发执行
func execDependencyTask(taskModel monitor.SysJob, taskResult TaskResult) {
	// 父任务才能执行子任务
	//if taskModel.Level != models.TaskLevelParent {
	//	return
	//}
...
// 发送任务结果通知
func SendNotification(taskModel monitor.SysJob, taskResult TaskResult) {
...
// 执行具体任务
func execJob(handler Handler, taskModel monitor.SysJob, taskUniqueId int64) TaskResult {
	defer func() {
	...
	return TaskResult{Result: output, Err: err, RetryTimes: 100}
}
